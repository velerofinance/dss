...
contract Vatish is DSToken('') {
    ...
    function usdv(address usr) public view returns (uint) {
...
contract FlopTest is DSTest {
    ...
    function test_dent() public {
        ...
        assertEq(vat.usdv(ali), 190 ether);
        // gal receives payment
        assertEq(vat.usdv(gal),  10 ether);
        assertEq(Gal(gal).Ash(), 0 ether);

        Guy(bob).dent(id, 80 ether, 10 ether);
        // bid taken from bidder
        assertEq(vat.usdv(bob), 190 ether);
        // prev bidder refunded
        assertEq(vat.usdv(ali), 200 ether);
    ...
    function test_yank() public {
        ...
        assertEq(vat.usdv(ali), 200 ether);
        assertEq(vat.usdv(bob), 200 ether);
        assertEq(vat.usdv(gal), 0);
        ...
        assertEq(vat.usdv(ali), 200 ether);  // ali's usdv balance is unchanged
        assertEq(vat.usdv(bob), 190 ether);
        assertEq(vat.usdv(gal),  10 ether);
        ...
        // confirm final state
        assertEq(vat.usdv(ali), 200 ether);
        assertEq(vat.usdv(bob), 200 ether);  // bob's bid has been refunded
        assertEq(vat.usdv(gal),  10 ether);
