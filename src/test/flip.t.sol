...
contract Vat_ is Vat {
    ...
    function usdv_balance(address usr) public view returns (uint) {
        return usdv[usr];
    }
    bytes32 ilk;
...
contract FlipTest is DSTest {
    ...
    function test_tend() public {
        ...
        assertEq(vat.usdv_balance(ali),   199 ether);
        // gal receives payment
        assertEq(vat.usdv_balance(gal),     1 ether);

        Guy(bob).tend(id, 100 ether, 2 ether);
        // bid taken from bidder
        assertEq(vat.usdv_balance(bob), 198 ether);
        // prev bidder refunded
        assertEq(vat.usdv_balance(ali), 200 ether);
        // gal receives excess
        assertEq(vat.usdv_balance(gal),   2 ether);
    ...
    function test_dent() public {
        ...
        assertEq(vat.usdv_balance(ali),  150 ether);
        assertEq(vat.usdv_balance(bob),  200 ether);
    }
    function test_tend_dent_same_bidder() public {
       uint id = flip.kick({ lot: 100 ether
        ...
        assertEq(vat.usdv_balance(ali), 10 ether);
        Guy(ali).tend(id, 100 ether, 200 ether);
        assertEq(vat.usdv_balance(ali), 0);
    ...
    function test_yank_tend() public {
        ...
        assertEq(vat.usdv_balance(ali),            200 ether);
        assertEq(vat.usdv_balance(address(this)),    0 ether);
