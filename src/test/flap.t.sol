...
contract FlapTest is DSTest {
    ...
    function test_kick() public {
        assertEq(vat.usdv(address(this)), 1000 ether);
        assertEq(vat.usdv(address(flap)),    0 ether);
        flap.kick({ lot: 100 ether
                  , bid: 0
                  });
        assertEq(vat.usdv(address(this)),  900 ether);
        assertEq(vat.usdv(address(flap)),  100 ether);
    }
    function test_tend() public {
        ...
        assertEq(vat.usdv(address(flap)),  0 ether);
        assertEq(vat.usdv(bob), 100 ether);
        // income is burned
