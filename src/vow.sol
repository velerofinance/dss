...
contract Vow {
    ...
    function heal(uint rad) external {
        require(rad <= vat.usdv(address(this)), "Vow/insufficient-surplus");
    ...
    function flop() external returns (uint id) {
        ...
        require(vat.usdv(address(this)) == 0, "Vow/surplus-not-zero");
        Ash = add(Ash, sump);
        id = flopper.kick(address(this), dump, sump);
    }
    // Surplus auction
    function flap() external returns (uint id) {
        require(vat.usdv(address(this)) >= add(add(vat.sin(address(this)), bump), hump), "Vow/insufficient-surplus");
    ...
    function cage() external auth {
        ...
        Sin = 0;
        Ash = 0;
        flapper.cage(vat.usdv(address(flapper)));
        flopper.cage();
        vat.heal(min(vat.usdv(address(this)), vat.sin(address(this))));
    }
}
